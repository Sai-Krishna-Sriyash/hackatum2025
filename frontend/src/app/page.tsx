				...
				<div className="relative">
					...
					<div className="relative bg-white border-2 border-[#D4AF37] rounded-2xl p-6 shadow-lg flex flex-col md:flex-row items-center gap-6">
						<div className="flex-1 space-y-3 text-center md:text-left">
							<div className="inline-flex items-center gap-2 bg-[#D4AF37] text-[#163C5D] px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wide">
								<Star size={12} fill="#163C5D" /> Monthly City Event
							</div>
							<h3 className="text-2xl font-bold text-[#163C5D]">{cityEvent.title}</h3>
							<p className="text-gray-600 text-sm">{cityEvent.description}</p>
							<button
								onClick={() => setSelectedEvent(cityEvent)}
								className="bg-[#163C5D] text-white px-6 py-2 rounded-lg font-bold text-sm shadow hover:bg-[#2D7A83] transition w-full md:w-auto"
							>
								View Official Event
							</button>
						</div>
						<div className="w-full md:w-64 h-45 rounded-xl overflow-hidden shadow-md flex-shrink-0">
							<img src={cityEvent.image} alt="City Event" className="w-full h-full object-cover" />
						</div>
					</div>
				</div>
			) : (
				<div className="text-gray-600 text-sm">Loadinggg....</div>
			)}
